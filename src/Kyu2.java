...
public class Kyu2 {
    public static long findPosition(final String s) {
        ...
        int eliminatedChars = 0;
        while(result == -1){
            currentNumber++;
            currentString += currentNumber;
            if(!currentString.contains(s)){
                int charactersToRemove = Long.toString(currentNumber).length();
                eliminatedChars += charactersToRemove;
                currentString = currentString.substring(charactersToRemove);
            }else{
                result = eliminatedChars + currentString.indexOf(s);
            }
        }
